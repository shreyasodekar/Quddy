...
for y in tqdm(range(len(y_pts))):
    ...
    ST_data = pna.polar()
    pna.output(0)
    # popt, pcov = curve_fit(fitter.lorentzian, ST_x_pts, 20*np.log10(np.abs(ST_data)), p0=[ST_x_pts[np.argmin(20*np.log10(np.abs(ST_data)))], -20, -10, 1e6] )
    # print('new resonator frequency is '+str(popt[0]/1e9) + ' GHz')
    # pna.start(popt[0] +5e3)
    # pna.stop(popt[0] +5e3)
    pna.start(ST_x_pts[np.argmin(20*np.log10(np.abs(ST_data)))])
