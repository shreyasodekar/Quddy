...
config['qubit']['wait_time'] = 0.001
config['qubit']['dephase_time'] = 0.5
...
#Actual Measurement
if Use_Raverager:
    prog = RamseyProgram(soccfg, config)
    ...
    for x in tqdm(range(len(x_pts))):
        ...
        prog = Programs.Ramsey(soccfg, config)
...
if show_plot:
    ...
    popt, pcov = curve_fit(fitter.sinedecay, x_pts, data.real, p0=[0, 0.02, 50, 0.5, 0])
    fig = plt.figure(figsize=(16,6))
    plt.subplot(121,title=r"T2$^*$ Ramsey", xlabel=r"$\tau$ ($\mu$s)", ylabel="Amp. (adc level)")
