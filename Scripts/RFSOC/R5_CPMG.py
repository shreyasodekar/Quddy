...
#Actual Measurement
if Use_Raverager:
    config['qubit']['num_pi_pulses_in_CPMG'] = 4
    prog = LoopbackPrograms.CPMG.CPMGProgram(soccfg, config)
...
plt.subplot(121,title=r"T2 CPMG", xlabel=r"Time ($\mu$s)", ylabel="Amp. (adc level)")
plt.plot(2*x_pts + 4*config['qubit']['sigma']*config['qubit']['num_pi_pulses_in_CPMG'], data.real, '.-')
