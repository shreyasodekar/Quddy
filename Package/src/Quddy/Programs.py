...
class Ramsey(AveragerProgram):
    def initialize(self):
        ...
        self.add_gauss(ch=cfg['qubit']['channel'], 
                       ...
                       length=self.us2cycles(cfg['qubit']['pulse_length']/2) # pi/2 pulse
                       )
...
class HahnEcho(AveragerProgram):
    def initialize(self):
        ...
        self.add_gauss(ch=cfg['qubit']['channel'], 
                       name="Ry", 
                       sigma=self.us2cycles(cfg['qubit']['pulse_length']/4), 
                       length=self.us2cycles(cfg['qubit']['pulse_length']) # pi pulse
                       ) 
    ...
    def body(self):
        ...
        self.sync_all(self.us2cycles(cfg['qubit']['dephase_time']/2)) # align channels and wait
...
class CPMG(AveragerProgram):
    ...
    def body(self):
        ...
        self.sync_all(self.us2cycles(cfg['qubit']['dephase_time']/(2*cfg['CPMG order']))) # align channels and wait
            
        for i in range(self.cfg['CPMG order']):    
            self.pulse(ch=self.cfg['qubit']['channel'])    #play pi probe pulse
            if i == self.cfg['CPMG order']-1:
                self.sync_all(self.us2cycles(cfg['qubit']['dephase_time']/(2*cfg['CPMG order']))) # align channels and wait
            else:
                self.sync_all(self.us2cycles(cfg['qubit']['dephase_time']/cfg['CPMG order'])) # align channels and wait
...
class Threetone(AveragerProgram):
    def initialize(self):
        ...
        self.add_gauss(ch=cfg['qubit']['channel'], 
                       name="qubit", 
                       sigma=self.us2cycles(cfg['qubit']['pulse_length']), 
                       length=self.us2cycles(cfg['qubit']['pulse_length'])*4)
...
class CPMG_loop(AveragerProgram):
    def initialize(self):      
        cfg=self.cfg
        self.declare_gen(ch=cfg['resonator']['channel'], nqz=cfg['resonator']['nqz']) #Readout
        self.declare_gen(ch=cfg['qubit']['channel'], nqz=cfg['qubit']['nqz']) #Qubit
